...
<script>
var target_goal = "Wealth Creation";
var goal_name = "Wealth Creation";
var amount = "";
var age = 30;
var horizon = "";
var risk = "";
...
$(document).ready(function(){
	...
	$("#txt_current_age").keypress(function(event) {
        // Allow only backspace and delete
        if ( event.keyCode == 46 || event.keyCode == 8 ) {
            // let it happen, don't do anything
        }
...
function showSummary()
{
	...
	}else
	{
		...
		if(folio_array.length == 0)
		{
			remaining_goal_amount = parseInt(target_amount);
			remaining_sip_amount = parseInt(sip_amount);
			str += '<tr class="goal_ques" style="font-weight: bold;" ><td>Monthly SIP investment required</td><td><input type="text" style="font-weight: bold;" id="txt_goal_sip_amount" value="'+numbersWithComma(sip_amount.toFixed(0))+'" class="form-control form-control-sm fundNamebg"></td></tr>';
		...
		}else
		{
			...
			}else
			{
				...
				str += '<tr class="goal_ques"><td style="font-weight: bold;">Monthly SIP investment required</td><td><input type="text" style="font-weight: bold;" id="txt_goal_sip_amount" value="'+numbersWithComma(remaining_sip_amount.toFixed(0))+'" class="form-control form-control-sm fundNamebg" ></td></tr>';
			}		
			$('#existing_summary_inner_div').append(str);
		}
	}
...
function calculateDebtAndEquity()
{
	...
	}else
	{
		...
		$('#old_robo2').addClass('col-md-3');
		$('#old_robo3').removeClass('col-md-2');
		$('#old_robo3').addClass('col-md-2');
		$('#old_robo4').show();
		$('#old_robo6').show();
...
function showRecommendedFunds()
{
	...
	html += '<table id ="wealthcreationtable">';
	html += '<thead>';
	html += '<tr>';
	...
	html += '<a id="return1" class="active" onclick="returnFilter(1)" > 1Y </a>'; 
	html += '<a id="return3" class="" onclick="returnFilter(3)"> 3Y </a>';
	html += '<a id="return5" class="" onclick="returnFilter(5)"> 5Y </a>'; 
	html += '</div></th>'; 
	html += '<th class="">ANALYSIS</th>';	
	html += '<th class="">ALLOCATION (%)</th>';	
	...
	for(k=0;k<cat_rolling_array.length;k++)
    {
		...
		$.post("/goal/getFundRatingBasedSchemes", {category : category, max_count : max_count}, function(data)
	    {
			...
			for (var i = 0; i < schemePerformances_array.length ; i++)
		    {
				...
				html += '<td class="schemenamethtd" style="width: 40.33% !important;"><span id="span_scheme_'+loop_count+'">' + scheme_name + '</span> | <span><a style="font-size: 10px;" href="javascript:;" onclick="schemeChange('+loop_count+', \''+category+'\', \''+scheme_code+'\')">Change Scheme</a></span></td>';
				html += '<td class="" style="width: 16% !important;" id="txt_category_'+loop_count+'">' + category + '</td>';
				html += '<td class="" style="text-align: right !important;"> '+ price +' </td>';	
...
{
	$('#'+id).highcharts({
		colors: ['#038bbb', '#003399', '#f58634', '#00a85b', '#858688', '#1cb7eb', '#FF9655', '#1686b0', '#6AF9C4'],
		chart: {
			type: 'area',
			style: {
				fontFamily: '"Open Sans", sans-serif',
...
{	
	...
	{ */
		 var table = document.getElementById('wealthcreationtable');
...
function checkLoginDetails()
{
	...
	$.post("/clientValidateLogin", {txt_pan : ""+pan+"", txt_pass : ""+pass+""}, function(data)
    {
		...
		function(){
   			top.location = "/goal/grow-money?amount="+amount+"&horizon="+horizon+"&inflation="+inflation+"&risk="+risk+"&goal_name="+goal_name;
		});
		return;
...
function txtyears() 
{
//	$(".childp1").css('visibility', 'visible');
	$(".childp1").show();
}
function txtreturn()
{
	$(".childp2").show();
	  var fieldVal = document.getElementById('txt_inflation').value;
	   ...
	   {
	       	var str = document.getElementById('txt_return').value;
	       str = str.substring(0, str.length - 1);
	   		document.getElementById('txt_return').value = str;
	   }
}
...
function returnFilter(retrunType) {
    ...
    <section class="page-title page-title-layout5">
      ...
      <div class="container">
        <div class="row">
          <div class="col-12">
            <h1 class="pagetitle__heading">Wealth Creation</h1>
            <nav>
              <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="/">Home</a></li>
                <li class="breadcrumb-item"><a href="/tools-and-calculators/become-a-crorepati">Goals</a></li>
                <li class="breadcrumb-item active" aria-current="page">Wealth Creation</li>
              </ol>
            </nav>
          </div><!-- /.col-12 -->
        </div><!-- /.row -->
    ...
    <section class="team-layout1 pb-20">
      <div class="container">
		...
		<div class="title-v1">
			<h1 class="text-center">Wealth Creation</h1>
		</div>
		</div>
		</div>
		...
		<p>You are <input id="txt_current_age" maxlength="2" class="form-control form-control-sm ml-2 w-15 w-custom-56 roboInput textInputRadius" style="width: 5%;color: #529100;font-weight: bold;font-size: 16px !important;" value="" type="text"> 
		 years old now you require Rs 
		<input id="txt_amount" maxlength="10" class="form-control form-control-sm ml-2 w-15 w-custom-56 roboInput textInputRadius number" style="width:20%;color: #529100;font-weight: bold;font-size: 16px !important;" value="" type="text">
		 at today's value after <input id="txt_years" maxlength="2" class="form-control form-control-sm ml-2 w-15 w-custom-56 roboInput textInputRadius" style="width:7%;color: #529100;font-weight: bold;font-size: 16px !important;" value="" type="text" onkeydown="txtyears()">
		...
		<input id="txt_inflation" maxlength="2" class="form-control form-control-sm ml-2 w-15 w-custom-56 roboInput textInputRadius" style="width:7%;color: #529100;font-weight: bold;font-size: 16px !important;" value="" type="number" oninput="checkLengthinflation()">
		 % and expect  
		<input id="txt_return" maxlength="2" class="form-control form-control-sm ml-2 w-15 w-custom-56 roboInput textInputRadius" style="width:7%;color: #529100;font-weight: bold;font-size: 16px !important;" value="" type="number" onkeydown="txtreturn()" oninput="txtreturn()">
		 % return on your investments.</p>
		</div>
		</div>
		</div>
		</div>
		...
		<div class="col-md-12 form-inline">
		<div class="row" style="margin:0px;margin-bottom:3%;">
		<div class="col-md-12 text-alignment" style="float:none;margin:0 auto;">
		<p>You are <input id="txt_current_age" maxlength="2" class="form-control form-control-sm ml-2 w-15 w-custom-56 roboInput textInputRadius" style="width: 5%;color: #529100;font-weight: bold;font-size: 16px !important;" value="" type="text"> 
		 years old now you require Rs 
		<input id="txt_amount" maxlength="10" class="form-control form-control-sm ml-2 w-15 w-custom-56 roboInput textInputRadius number" style="width:20%;color: #529100;font-weight: bold;font-size: 16px !important;" value="" type="text">
		 at today's value after 
		<input id="txt_years" maxlength="2" class="form-control form-control-sm ml-2 w-15 w-custom-56 roboInput textInputRadius" style="width:7%;color: #529100;font-weight: bold;font-size: 16px !important;" value="" type="text">
		 years for becoming wealthy. <br>You assume the inflation to be 
		<input id="txt_inflation" maxlength="2" class="form-control form-control-sm ml-2 w-15 w-custom-56 roboInput textInputRadius" style="width:7%;color: #529100;font-weight: bold;font-size: 16px !important;" value="" type="text">
		 % and expect  
		<input id="txt_return" maxlength="2" class="form-control form-control-sm ml-2 w-15 w-custom-56 roboInput textInputRadius" style="width:7%;color: #529100;font-weight: bold;font-size: 16px !important;" value="" type="text">
		 % return on your investments.</p> 
		...
		<div class="row" style="margin:0px;margin-bottom:3%">
		<div class="col-md-12" style="float:none;margin:0 auto;">
		<p>The cost of this House would be around &nbsp;<i class="fa fa-inr"></i> 
		<input id="txt_amount" maxlength="13" class="form-control form-control-sm w-15 w-custom-56 roboInput textInputRadius number" style="width:17%" value="" type="text" onchange="addCommas()">
		in today's value.</p>
		</div>
		</div>
		</div>
		...
		<div class="col-md-12" style="float:none;margin:0 auto;">
		<p>You assume the inflation to be 
		<input id="txt_inflation" maxlength="2" class="form-control form-control-sm w-15 w-custom-56 roboInput textInputRadius" style="width:5%" value="" type="text">
		 % and You can take 
		<select id="" class="form-control form-control-sm w-25 w-custom-50 roboInput textInputRadius" onchange="riskProfileChange()" style="text-align:center;text-align-last:center;width:23%;padding:0px;">
		...
		<div class="col-md-12" style="float:none;margin:0 auto;">
		<p>You can take 
		<select id="sel_risk" class="form-control form-control-sm w-25 w-custom-50 roboInput textInputRadius" onchange="riskProfileChange()" style="text-align:center;text-align-last:center;width:23%;padding:0px;font-weight: bold; font-size: 16px !important">
		<option value="Conservative">Conservative</option>
		<option value="Moderately Conservative">Moderately Conservative</option>
		<option value="Moderate">Moderate</option>
		<option value="Moderately Aggressive">Moderately Aggressive</option>
		...
		<option value="">Check Your Risk Profile</option>
		 ...
		 <div class="row pt4" style="margin:0px; display: None !important;" >	  
		<div class="col-md-12 form-inline" style="float:none;margin:0 auto;">
		<label>You would like to name this goal as  
		<input id="txt_goal_name" maxlength="25" class="form-control form-control-sm w-15 w-custom-56 roboInput" style="width:20%;" value="Wealth Creation" type="text">
		</label>
		</div>
		</div>
		
		<div class="row pt4">
		<div class="col-md-12 text-center py-5" style ="margin-top: -4% !important;">	
		<button class="btn btn__secondary btn__rounded btnCol" style="margin:0 auto; float: none;" onclick="checkResult()">Build My Wealth Creation Goal</button>
		</div>
		</div>
			 ...
			 <div class="row text-center">	  
			  ...
			  <div class="row">
			   <div class="col-md-12 col-sm-12 pt-4">	   
				 ...
				 <button class="btn btn-primary col-lg-custom btn-md btnCol col-lg-border-custom" style="width: auto; background-color: #fff;border-color: #6ac259;font-weight: bold;color: #080808; " onclick="goToStep1()">Back</button>
		  	...
		  	<div class="row">
				<div class="col-lg-12 pt-4">
				    ...
				    <div class="row">
					 ...
					 <div class="row py-4">
						 <div class="col-lg-12 text-center pb-4"  style=" margin-top: 5% !important;">
							 <button class="btn btn-primary col-lg-custom btn-md btnCol col-lg-border-custom" onclick="hide_div(2)" style="width: auto; background-color: #20C4B7;border-color: #6ac259;color: #fff;  font-weight: bold;">Back</button>
		...
		<!-- <img src="resources/img/back_img.png" alt="" style="width:40px;" /> -->
		</a>
		</div>
		</div>
		</div>
		...
		<div class="row">
			...
			<div class="col-lg-6 col-sm-4 text-center mb-3">
				<div class="box bg-tf p-3">
					...
					<p class="goalcontentbg">Future value of your Dream Home<br>(adjusting for <span class="span_inflation"></span>% inflation)</p>
				</div>
			</div>
			<div class="col-lg-6 col-sm-4 text-center mb-3">
				<div class="box bg-tf p-3">
		...
		<div class="col-lg-12 text-right">
		<button class="btn btn-primary" style="width: auto; background-color: #20C4B7;border-color: #20C4B7;color: #fff;  font-weight: bold;" onclick="goToStep2()">Back</button>
		...
		<div class="title">Total Amount <span  style="margin-left: 12px; font-size: 16px;"><i class="fa fa-inr"></i></span><span id="totAmt"  style="color: #2e3e4f; margin-left: 6px; font-size: 20px;
		...
		<c:otherwise>
			...
			<button type="submit" class="btn btn-success btnfloat" onclick="showLogin(2)" style="max-width: 100%;">Save Your Goal</button><!-- </div>
			<div class="col-lg-3">
		...
		<c:otherwise>
			<div class="col-lg-6 pl-3 pr-3" style="padding-right:0px;">
  ...
  <div class="modal-lg modal-dialog modal-dialog-centered">
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
		<button type="button" class="btn btn-primary" data-dismiss="modal" style="width: auto; background-color: #20C4B7;border-color: #6ac259;color: #fff;  font-weight: bold;">Close</button>
...
<div id="login_model" class="modal fade" tabindex="-1" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-dialog-centered" style="max-width:500px !important;">
    ...
    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header" style = "background: #6ac259;color: white;">Login</div>
      <div class="modal-body">
